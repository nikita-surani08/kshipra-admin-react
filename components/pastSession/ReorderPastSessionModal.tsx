...
interface ReorderPastSessionModalProps {
  visible: boolean;
  sessions: PastSession[];
  onCancel: () => void;
  onSave: (reorderedSessions: PastSession[]) => void;
  loading?: boolean;
  subjectName?: string;
  topicName?: string;
}

interface DraggablePastSessionCardProps {
  session: PastSession;
  index: number;
  moveSession: (dragIndex: number, hoverIndex: number) => void;
}

const DraggablePastSessionCard: React.FC<DraggablePastSessionCardProps> = ({ session, index, moveSession }) => {
...
const ReorderPastSessionModal: React.FC<ReorderPastSessionModalProps> = ({
  visible,
  sessions,
  onCancel,
  onSave,
  ...
  const [reorderedSessions, setReorderedSessions] = useState<PastSession[]>([]);
  ...
  const getModalTitle = () => {
    if (subjectName && topicName) {
      return `Reorder Past Sessions: ${subjectName} > ${topicName}`;
    } else if (subjectName) {
      return `Reorder Past Sessions: ${subjectName}`;
    } else {
      return "Reorder Past Sessions";
    }
  };
...
export default ReorderPastSessionModal;
